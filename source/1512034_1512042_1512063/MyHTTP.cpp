...
void MyHTTP::writeData(QueueURI& uris, const int& versionHTTP, const bool& hasChunked)
{
	...
	strcat(full_item_name, this->m_URI.pre_ItemName);
	if (strcmpi(full_item_name, "1512034_1512042_1512063_") != 0)
		strcat(full_item_name, "/");
	strcat(full_item_name, this->m_URI.itemname);
	int length = 0;
	...
	if (this->m_URI.isFile)
	{
		...
		if (myFile)
		{
			cout << "*** Downloading file: " << this->m_URI.itemname << ": ";
	...
	else
	{
		...
		cout << "@@ Downloading folder: " << this->m_URI.itemname << endl;
